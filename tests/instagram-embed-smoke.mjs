...
try {
	for (const viewport of [
		...
		await page.route(EMBED_URL, (route) =>
			route.fulfill({ status: 200, contentType: "text/html", body: EMBED_HTML }),
		);

		await page.goto(`${SITE}/portraits/`, { waitUntil: "load" });
		const tile = page.getByRole("button", { name: /View photo: Collaborator carousel/i });
		await tile.waitFor();
		assert.equal(await page.locator("iframe").count(), 0, "the grid must not preload an embed");
		...
		assert.equal(await dialog.locator("img").count(), 0, "embed fallback must replace the custom lightbox image");
		...
		assert(bounds.x + bounds.width <= viewport.width, "embed overflows viewport width");
		assert(bounds.y + bounds.height <= viewport.height, "embed overflows viewport height");
